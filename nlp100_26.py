...
with open("uk.txt", mode="r")as f:
    ...
    keys = [re.sub(r'\*+', '', field_value[i][0]) for i in range(len(field_value))]  # keyの強調マークアップの除去
    values = [re.sub(r'\*+', '', field_value[i][1])for i in range(len(field_value))]  # valueの強調マークアップの除去
