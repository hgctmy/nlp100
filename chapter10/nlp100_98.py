...
if __name__ == "__main__":
    train_jescdata = CustomDataset("split/train.ja.wakati", "split/train.en")
    ...
    train_jescdataloader = DataLoader(train_jescdata, batch_size=32, shuffle=True, collate_fn=collate_fn)
    ...
    for t in range(5):
        print(f"Epoch {t+1}\n-------------------------------")
        train_epoch(train_jescdataloader, model, optimizer)
    print("Done!")

    torch.save(model.state_dict(), 'transformer_weights_jesc.pth')
    ...
    torch.save(model.state_dict(), 'transformer_weights_kftt.pth')
