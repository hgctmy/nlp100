...
with open("uk.txt", mode="r")as f:
    ...
    keys = [re.sub(r"[*[\](''')]", '', field_value[i][0]) for i in range(len(field_value))]  # keyのマークアップの除去
    values = [re.sub(r"[*[\](''')]", '', field_value[i][1]) for i in range(len(field_value))]  # valueのマークアップの除去
